...
### 合规修复
- 中文与日文官网首页页脚居中展示网站服务备案号“京ICP备2025150534号-1”，点击可前往工信部备案查询页。

## [2026-09-19]

### 新功能
- 新增与国内站隔离的日本海外版官网 `/jp/`，保留品牌加载页、首页内容和背景音乐，并加入首页 / 商品双入口导航。
- 新增日本版商品页：展示 PRODUCT_01 白色与紫色款、多视角素材、产品规格、双版本说明书、安全说明和一年保修内容。
- 新增商品发售通知预约：邮箱写入独立后台名单，支持同邮箱去重、重复预约提示、隐私说明和退订入口。

### 体验调整
- 统一日本版首页与商品页的导航、Logo、音量按钮、页面宽度和缩放；补充滚动顶部遮罩、章节分隔线、标题光晕及进入视口动效。
- 日本版社交入口调整为 TikTok、Instagram 和 X，并统一三枚图标的视觉尺寸。
...
### 调整
- 内测预约「中选结果页」改造：由「扫码加入内测群」改为引导添加**小月个人微信**（新增 `images/beta-wechat-xiaoyue.jpg` 二维码）；新增按所查手机号**自动生成**的「验证消息 / 打招呼」暗号「月光茧内测+手机号后四位」并带一键复制按钮（后四位由前端按归一化手机号实时计算，用户无需自行拼写）。中选页二维码改为前端内置图，不再依赖后端 `groupQrUrl`。

## [2026-06-24]

### 调整
- 内测预约问卷链接更新为新的飞书表单（`BETA_FORM_URL` 换 ID，替换旧表单）。
...
### 调整
- 文案校对修正（静态 markup 与 moduleData 两处副本同步）：跨次元互动列举改用顿号「文字、语音、图片、文件、链接」；独家记忆首句去掉句末多余逗号；人设定制首句「结合用户性格与喜好，设定他的性格」改为「结合你的喜好与个性，设定他的性格」消除「性格」重复。
